...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        # Tinh chỉnh theo kênh (Channel Refinement)
        out = x * self.ca(x)
        # Tinh chỉnh theo không gian (Spatial Refinement)
        result = out * self.sa(out)
        return result

class RedChannelPrior(nn.Module):
    def __init__(self, ch_in=3, patch_size=15, embed_dim=32):
        super(RedChannelPrior, self).__init__()
        self.patch_size = patch_size
        self.pad = patch_size // 2
        self.enc = nn.Sequential(
            # 1. Tăng số kênh
            nn.Conv2d(ch_in, embed_dim, 3, 1, 1),
            nn.LeakyReLU(0.1, inplace=True),
            
            # 2. Áp dụng CBAM để tinh chỉnh đặc trưng RCP
            # CBAM giúp mạng biết: "Kênh nào quan trọng?" và "Vùng nào là vật thể?"
            CBAM(embed_dim), 
            
            # 3. Giảm về số kênh gốc (để khớp với input của FeatureContextualizer)
            nn.Conv2d(embed_dim, ch_in, 3, 1, 1)
        )
